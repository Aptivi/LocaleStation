    ...
    public class LocalizationGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(localizationLanguageFiles.Combine(localizationCompilation), (ctx, pair) =>
            {
                ...
                // First, convert the individual strings to their compilable representations using dictionaries
                var strings = localizationJson.Localizations;
                List<string> processedLocalizationNames = [];
                langClassBuilder.AppendLine("        private static Dictionary<string, string> localizations = new Dictionary<string, string>()");
                langClassBuilder.AppendLine("        {");
                foreach (var str in strings)
                {
                    string finalLocalizationName = str.Localization.ToUpper();
                    int repetitions = 1;
                    ...
                    langClassBuilder.AppendLine($"            {{ \"{finalLocalizationName}\", @\"{str.Text.Replace("\"", "\"\"")}\" }},");
                }
                langClassBuilder.AppendLine("        };");
                langClassBuilder.AppendLine();
                ...
                langClassBuilder.AppendLine("        {");
                langClassBuilder.AppendLine("            if (localizations.TryGetValue(loc, out string locString))");
                langClassBuilder.AppendLine("                return locString;");
                ...
                if (processedLocalizationNames.Count == 0)
                    ...
                    langClassBuilder.AppendLine("                localizations.TryGetValue(loc, out _)");
                langClassBuilder.AppendLine("            ;");
                langClassBuilder.AppendLine("        }");
